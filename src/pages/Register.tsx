...
export default function Register() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data?.user) {
        alert('УРА! Пользователь успешно создан в Supabase!');
        navigate('/login');
      }
    } catch (err: any) {
      alert('Ошибка: ' + (err.message || 'Сбой регистрации'));
    ...
    <div className="min-h-screen bg-[#0a0a0a] flex items-center justify-center p-4">
      <div className="bg-[#141414] p-8 rounded-xl border border-neutral-800 max-w-md w-full text-white">
        ...
        <form>
          ...
          <button type="button" onClick={(e) => handleSubmit(e)} disabled={loading} className="w-full bg-amber-500 hover:bg-amber-600 text-black font-bold p-3 rounded transition duration-200 disabled:opacity-50">
